...
LightCollectionDetectorMessenger::LightCollectionDetectorMessenger(
  ...
  : G4UImessenger()
  , fLightCollectionDetCon(detcon)
{
  fDetConDir = new G4UIdirectory("/LightCollection/DetectorConstruction/");
  fDetConDir->SetGuidance("Configuring Detector Construction");

  fVerboseCmd =
    new G4UIcmdWithABool("/LightCollection/DetectorConstruction/enableVerbose", this);
  ...
  fDumpGdmlCmd =
    new G4UIcmdWithABool("/LightCollection/DetectorConstruction/dumpGdml", this);
  ...
  fDumpGdmlFileNameCmd = new G4UIcmdWithAString(
    "/LightCollection/DetectorConstruction/dumpGdmlFileName", this);
  fDumpGdmlFileNameCmd->SetGuidance("Enter file name to dump gdml file ");
  fDumpGdmlFileNameCmd->SetDefaultValue("LightCollection_dump.gdml");
...
void LightCollectionDetectorMessenger::SetNewValue(G4UIcommand* command,
                                            G4String newValue)
{
  LightCollectionDetectorConstruction* dc1 =
    dynamic_cast<LightCollectionDetectorConstruction*>(fLightCollectionDetCon);
  if(dc1 != nullptr)
  {
  ...
  {
    LightCollectionGDMLDetectorConstruction* dc2 =
      dynamic_cast<LightCollectionGDMLDetectorConstruction*>(fLightCollectionDetCon);
    if(command == fVerboseCmd)
