...
OpNoviceDetectorConstruction::OpNoviceDetectorConstruction()
  : G4VUserDetectorConstruction(),reflectorShape("HEMISPHERE"), scintillatorSize(G4ThreeVector(1*cm,1*cm,1*cm)), reflectorSize(G4ThreeVector(2*cm,2*cm,2*cm)), useGrease(1)
{
  messenger = new G4GenericMessenger(this, "/detector/", "Detector control");
  messenger->DeclareProperty("reflectorShape", reflectorShape, "Shape of the reflector");
  messenger->DeclareProperty("reflectorMaterial", reflectorMaterial, "Material of the reflector");
  messenger->DeclareProperty("scintillatorSize", scintillatorSize, "Size of the scintillator");
  messenger->DeclareProperty("reflectorSize", reflectorSize, "Size of the reflector");
  messenger->DeclareProperty("useGrease", useGrease, "Use grease under scintillator");
  ...
  scintillatorX = scintillatorSize.x();
  scintillatorY = scintillatorSize.y();
  scintillatorZ = scintillatorSize.z();


  // reflectorShape = TEFLON;
  reflectorShape = "HEMISPHERE";
  // reflectorShape = "CYLINDER";
  reflectorMaterial = "TiO2";
}
...
G4VPhysicalVolume* OpNoviceDetectorConstruction::Construct()
{
  ...
  switch (reflectorShapeMap[reflectorShape]) {
      case 1:   // 对应于 "CUBE"
        {
...
case 2: // CYLINDER
{
    ...
    MyPhysicalVolume* physReflectorBody = new MyPhysicalVolume(0, posReflectorBody, logicReflectorBody, "ReflectorBody", physWorld, false, 0);

    // 放置 Reflector 顶部
    G4ThreeVector posReflectorTop = G4ThreeVector(0, 0, posReflectorBody.z() + 0.5*height + 0.5*topThickness);
    MyPhysicalVolume* physReflectorTop = new MyPhysicalVolume(0, posReflectorTop, logicReflectorTop, "ReflectorTop", physWorld, false, 0);
    ...
    break;
      ...
      case 3: // HEMISPHERE
        { 
          ...
          // 定义 Reflector 的内外半径
          G4double innerRadius = 15*mm;
          G4double outerRadius = innerRadius + 3*mm;
          // 创建 Reflector
...
void OpNoviceDetectorConstruction::SetReflectorShape(G4String shape) {
    if (reflectorShapeMap.find(shape) != reflectorShapeMap.end()) {
        reflectorShape = shape;
    } else {
        // 错误处理：shape不在reflectorShapeMap中
    }
}

void OpNoviceDetectorConstruction::SetReflectorMaterial(G4String material) {
    if (reflectorMaterialMap.find(material) != reflectorMaterialMap.end()) {
        reflectorMaterial = material;
    } else {
        // 错误处理：material不在reflectorMaterialMap中
    }
}

void OpNoviceDetectorConstruction::SetScintillatorSize(G4ThreeVector size) {
    scintillatorSize = size;
}

void OpNoviceDetectorConstruction::SetReflectorSize(G4ThreeVector size) {
    reflectorSize = size;
}

void OpNoviceDetectorConstruction::UseGrease(G4bool use) {
    useGrease = use;
}