    ...
    : G4UserRunAction(), fRun(nullptr), fPrimary(prim)
{
  ...
  G4double gamma_eng = 40 * keV;        // 入射光子的能量
  G4double LY = 40 / keV;               // 光产额
  G4double xmax = gamma_eng * LY * 1.4; // 根据入射能量和光产额设置直方图的上限
  ...
  // Creating ntuple
  analysisManager->CreateNtuple("Light Yield Sim", "Photon Generated and Detected");
  analysisManager->CreateNtupleDColumn("NPhotonGenerated");
  analysisManager->CreateNtupleDColumn("NPhotonDetected");
...
bool OpNoviceRunAction::fileExists(const std::string& fileName)
{
    std::ifstream file(fileName.c_str());
    return file.good();
}

G4String OpNoviceRunAction::getNewfileName(G4String baseFileName)
{
